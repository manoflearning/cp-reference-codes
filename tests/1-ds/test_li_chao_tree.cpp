#include "../../src/1-ds/li_chao_tree.cpp"

// what: tests for li_chao (max line query).
// time: random + edge cases; memory: O(n)
// constraint: uses assert, fixed seed.
// usage: g++ -std=c++17 test_li_chao_tree.cpp && ./a.out
...
ll eval_line(const line &ln, ll x) { return ln.first * x + ln.second; }

ll max_naive(const vector<line> &lns, ll x) {
    ll ret = NEG_INF;
